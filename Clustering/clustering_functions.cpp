...
vector<Image*> kMeansInitialization(vector<Image*> images, vector<Image*> imagesOld, int iterations, int k, int numOfImages, int dimensions){
    ...
    while (t != k)
   	{
   		...
   		for(int i = 0; i < numOfImages; i++)
            ...
            {
       		   distances.push_back(manhattanDistance(images[centroidId]->getVal(), imagesOld[i]->getVal(), dimensions));
            }
            /////////////////////////////////////////////////////////////////////////////
        }
...
vector<Cluster *> LloydsAlgorithm(vector<Image *> images, vector <Image*> imagesOld, vector<Image *> centroids, int numOfImages)
{
    ...
    while (percentage > 0.5)
    {
        ...
        for (i = 0; i < numOfImages; i++)
        {
            ...
            for (c = 0; c < centroids.size(); c++)
            {
                ...
                int dist = manhattanDistance(centroids[c]->getVal(), imagesOld[i]->getVal(), images[i]->getDimensions());
                // find minimum
            ...
            images[i]->setCluster(id1);							// REPEAT THIS IN HALF LLOYD'S!!!
        ...
        for (int c = 0; c < clusters.size(); c++)
        {
            for (i = 0; i < numOfImages; i++)
            {
                if (imagesOld[i]->getCluster() == c)
                {      
                    (clusters[c]->getImagesVector())->push_back(imagesOld[i]->getVal());
                }
            }
        }
        /////////////////////////////////////////////////////////////////////////////
        
        int dimensions = centroids[0]->getDimensions();     // = 10
        ...
        {  
            ...
            for (i = 0; i < dimensions; i++)
            {
                ...
                if((*clusters[c]->getImagesVector()).size() == 0) continue;
        ...
        currentMedian = CalculateMedianDistance(imagesOld);
...
vector<Cluster *> HalfLloydsAlgorithm(vector<Image *> images, int numOfImages, vector<Cluster *> clusters){
    // Lloyd's algorithm but for preset clusters -> only to define centroid
	vector<Image *> updatedCentroids;
	vector<Image *> centroids;
    int dimensions = clusters.size();
    ...
    for (int c = 0; c < clusters.size(); c++){  
        ...
        for (int i = 0; i < dimensions; i++)
        {
            ...
            // keep the i-th pixels of all images that are assigned to the cluster
            for (int p = 0; p < (*clusters[c]->getImagesVector()).size(); p++)
            {
                uint8_t * tempImage = (*clusters[c]->getImagesVector())[p];
                newPixel.push_back(tempImage[i]);
            }
	...
	centroids = updatedCentroids;
    // assign these centroids to the preset clusters
	for (int i = 0; i < centroids.size(); i++)
	{
		cout << "yes" << endl;
		clusters[i]->setCluster(centroids[i]);
	}
    // assign new current centroids set and proceed to next loop
    return clusters;
}
...
void reverseAssignmentCube(vector <Image*> clusterCenters, vector <Image*> images, int M, int k, int probes){
    ...
    while(percentage > 0.5){
        ...
        for(int i=0; i<clusterCenters.size(); i++){
            int oldSize = -1;
            vector <Values> itemsInCluster;
            // cout << "outer: " << oldSize << " " << itemsInCluster.size() << endl;
            while(oldSize != itemsInCluster.size()){
                // cout << oldSize << "  " << itemsInCluster.size() << endl;
                ...
                for(int j=0; j<limit; j++){
                    ...
                    // cout << clusterSystem.size() << " " << neighborsInRange[j].getHashResult() << endl;
                    if(neighborsInRange[j].getFlag()<0 && neighborsInRange[j].getHashResult()>=0){
                        clusterSystem[neighborsInRange[j].getHashResult()].push_back(neighborsInRange[j]);
                    }
                }
...
vector<double> Silhouette(vector<Cluster *> clusterSet,vector<Image *> images){
    ...
    for (i = 0; i < images.size(); i++)
    {
        ...
        // if(clusterOfimage) cout << clusterOfimage << endl;
...
int manhattan(uint8_t * qImage, uint8_t * tempImage, int size){
    int distance = 0, i;
    for (i = 0; i < size; i++)
    {
...
// find average distance between image-centroid
double objectiveFunction (vector<Cluster *> clusters, int dimensions){
	double objFuncValue = 0.0;
	for (int i = 0; i < clusters.size(); ++i){
		double medianDist = 0.0;
		if((*clusters[i]->getImagesVector()).size() == 0) continue;
		for(int j = 0 ; j< (*clusters[i]->getImagesVector()).size(); j++){
			medianDist += manhattan((*clusters[i]->getImagesVector())[j], clusters[i]->getCluster()->getVal(), dimensions);
		}
		objFuncValue += (double)(medianDist)/(double)((*clusters[i]->getImagesVector()).size());
	}
	if(clusters.size()) objFuncValue /= (double)clusters.size();
	return objFuncValue;
}